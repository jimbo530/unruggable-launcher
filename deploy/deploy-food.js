...
// FOOD / edible trade-good tokens (Arms & Equipment Guide p31, thinned) + RATIONS.
// D&D prices; uniform $100M FDV (gold=$0.01) so supply = 1e10/goldValue (rounded).
// RATIONS = crew upkeep sink (1 per pawn per day at sea).
const COINS = [
  { name: 'Salt',     symbol: 'SALT',     whole: 1_000_000_000_000n }, // 1 cp  (0.01g)
  { name: 'Honey',    symbol: 'HONEY',    whole:   100_000_000_000n }, // 1 sp  (0.1g)
  { name: 'Rations',  symbol: 'RATIONS',  whole:    20_000_000_000n }, // 5 sp  (0.5g)
  { name: 'Apple',    symbol: 'APPLE',    whole:    10_000_000_000n }, // 1 gp
  { name: 'Cinnamon', symbol: 'CINNAMON', whole:    10_000_000_000n }, // 1 gp
  { name: 'Cod',      symbol: 'COD',      whole:     2_000_000_000n }, // 5 gp
  { name: 'Jerky',    symbol: 'JERKY',    whole:     1_430_000_000n }, // 7 gp
  { name: 'Ale',      symbol: 'ALE',      whole:     1_250_000_000n }, // 8 gp
  { name: 'Pepper',   symbol: 'PEPPER',   whole:       333_000_000n }, // 30 gp
  { name: 'Wine',     symbol: 'WINE',     whole:       250_000_000n }, // 40 gp
  { name: 'Saffron',  symbol: 'SAFFRON',  whole:       154_000_000n }, // 65 gp
];

const OUT = path.join(__dirname, 'foods-deployed.json');
...
async function main() {
  ...
  // Explicit fee bump — clears any stuck/underpriced tx at the current nonce and clears
  // Base basefee with margin (per tx-pacing notes). Still pennies at Base gas levels.
  const fees = { maxFeePerGas: ethers.parseUnits('0.1', 'gwei'), maxPriorityFeePerGas: ethers.parseUnits('0.02', 'gwei') };
  // Explicit nonce — Alchemy's read can lag behind the sequencer. START_NONCE overrides;
  // otherwise use the sequencer's pending count. Increment per deploy.
  ...
  for (const c of COINS) {
    ...
    console.log(`Deploying ${c.symbol} ... (nonce ${nextNonce})`);
    const token = await factory.deploy(c.name, c.symbol, supply, treasury, '', { ...fees, nonce: nextNonce });
    nextNonce++;
