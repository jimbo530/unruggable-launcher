...
/// @title Unruggable2 V5.7 — MfT Money Floor + Dual Launch Tiers
///
/// Two launch modes:
///   - launchMeme():    $5 mftUSD min, no vesting, 100% supply to pools
///   - launchProject(): $100 mftUSD min, 20% VolumeVesting, 80% supply to pools
///
/// Floor asset is now mftUSD (Money for Trees stablecoin) instead of USDC.
/// For cbBTC/WETH diversification, factory withdraws mftUSD → USDC → swaps.
///
/// Pool structure per launch:
///   3 floor pools: TOKEN/mftUSD + TOKEN/cbBTC + TOKEN/WETH
///   3 MfT sell walls: TOKEN/MfT(meme) at 1.1x / 2x / 5x
///   CHAR reactor: CHAR/cbBTC + CHAR/WETH (6% of seed)
///
/// V5.7 changes from V5.6:
///   - mftUSD replaces USDC as seed currency and primary floor
///   - Two launch tiers: meme ($5, no vest) and project ($100, 20% vest)
///   - Factory calls mftUSD.withdraw() internally to get USDC for swaps
...
contract Unruggable2 {
    ...
    address public immutable mftStable;
    ...
    function _launchStep1(
        ...
        // Pull mftUSD from launcher
        IMftStable(mftStable).transferFrom(msg.sender, address(this), _seedAmount);
        ...
        // Split: 40% stays as mftUSD, 60% gets withdrawn to USDC for BTC/ETH swaps
        uint256 stableKeep  = floorSeed * SEED_STABLE / 10000;
        uint256 forWithdraw = floorSeed - stableKeep;

        // Withdraw mftUSD → USDC for cbBTC/ETH purchases
        IMftStable(mftStable).withdraw(forWithdraw);
        uint256 usdcForBtc  = forWithdraw * SEED_BTC / (SEED_BTC + SEED_ETH);
        uint256 usdcForWeth = forWithdraw - usdcForBtc;
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //  Internal — CHAR reactor setup (withdraw mftUSD → USDC → CHAR + cbBTC/WETH)
    // ═══════════════════════════════════════════════════════════════════════

    function _setupCharReactor(uint256 charSeed, address upstreamAddr) internal returns (address charReactorAddr) {
        // Withdraw mftUSD → USDC for CHAR purchases
        IMftStable(mftStable).withdraw(charSeed);

        uint256 usdcForChar  = charSeed / 2;
        uint256 usdcForPairs = charSeed - usdcForChar;
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //  Internal — create floor pool (TOKEN/mftUSD or TOKEN/cbBTC or TOKEN/WETH)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    /// @dev Derive MfT meme price for sell walls using WETH/MfT + USDC/WETH pools
    function _deriveMftPrice(
        bool tokenIs0InMftPool,
        uint256 floorTokens,
        uint256 floorStable6
    ) internal view returns (uint160) {
        // Get USDC/WETH price (mftUSD ≈ USDC, same 6 decimals)
    ...
    function _refundDust(address tokenAddr, address to) internal {
        address[7] memory tokens = [tokenAddr, weth, usdc, wrappedBtc, char, mft, mftStable];
        for (uint256 i; i < 7; ++i) {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //  Cancel pending launch (refunds remaining mftUSD held for CHAR)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    function cancelPending() external {
        ...
        require(p.token != address(0), "no pending");
        delete pending[msg.sender];
        uint256 bal = IMftStable(mftStable).balanceOf(address(this));
        if (bal > 0) IMftStable(mftStable).transfer(msg.sender, bal);
    }
