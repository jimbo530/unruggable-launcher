...
///
/// V5.1 changes from V5: USDC routed directly to each floor asset.
///   - 40% USDC -> AZUSD via Aerodrome (1 swap, was 3 in V5)
///   - 60% USDC -> WETH via Uniswap, then split WETH 50/50 to cbBTC and keep
///   Saves ~200K gas and eliminates double-swap slippage loss.
...
contract Unruggable2 {
    ...
    //  V5.1: direct routing — no double-swap
    // ═══════════════════════════════════════════════════════════════════════
    ...
    function _prepareFloorSeed(uint256 usdcAmount) internal returns (
        ...
        uint256 usdcForWeth  = usdcAmount - usdcForAzusd;         // 60%

        // 1. USDC -> AZUSD directly via Aerodrome (was USDC->WETH->USDC->AZUSD in V5)
        ...
        // 3. Split WETH: 50% -> cbBTC, 50% kept as WETH
        uint256 wethForBtc = wethTotal * SEED_BTC / (SEED_BTC + SEED_ETH);
        ethAmt = wethTotal - wethForBtc;

        IERC20(weth).approve(swapRouter, wethForBtc);
        btcAmt = ISwapRouter02(swapRouter).exactInputSingle(
            ISwapRouter02.ExactInputSingleParams({
                tokenIn: weth, tokenOut: wrappedBtc, fee: wethBtcFee,
                recipient: address(this), amountIn: wethForBtc,
                amountOutMinimum: 0, sqrtPriceLimitX96: 0
            })
        );
    }
