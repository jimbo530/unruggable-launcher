...
/// @title Shipyard — Free Launch of mutiny-capable ships (go-forward factory)
///
/// Clone of UnrugableV9 (MycoPadV9) with ONE change: it launches a ShipToken
/// instead of a frozen LaunchToken, and wires the token's crew to the launch's
/// FeeShareDistributor (the 100-NFT collection). The crew can later "mutiny" the
/// ship — rename/re-flag it (cosmetic only) by 51% NFT vote. Everything economic
/// is still immutable: fixed supply, locked LPs, burn-and-fee-share reactor.
///
/// Pipeline (identical to V9):
///   - 1,000,000,000 fixed supply, $10,000 market cap
///   - 70% supply → TOKEN/Money sell wall, 30% → TOKEN/Meme sell wall
///   - SporeReactorV6: burns 100% of collected TOKEN, redeems Money fees to USDC
///     into the crew distributor, deepens the Meme LP (LP-only, no upstream fuel)
///   - 100 crew NFTs minted to the launcher, money+usdc wired, dust refunded
///
/// IMPORTANT: reactorImpl MUST be a SporeReactorV6 impl. NEW LAUNCHES ONLY —
/// MycoPadV9 (frozen-token variant) is left untouched for reference.
...
interface IMoneyDeposit {
    /// @dev Verified on Base fork (block 47510000): deposit(uint256) mints Money
    ///      1:1 from USDC to the caller.
...
contract Shipyard {
    ...
    /// Capped at $5 so the fee can never look predatory; 0 allowed (free launch).
    ...
    /// A tiny "moonshot" buy at the $10K floor inside the launch tx. Effective
    /// spend each launch = min(buyInAmount, launchFee) — never spends more than
    /// the fee collected. Bought TOKEN goes to prizeWallet; remainder → treasury.
    ...
    ///         Thin wrapper over launchFor so there is a single pipeline.
    function launch(
    ...
    ///         The $1 USDC launch fee is pulled from msg.sender — the CALLER /
    ///         RELAYER fronts it (the app collects the $1 from the end user
    ///         off-chain). Mirrors the depositFor pattern: relayer pays, user owns.
    function launchFor(
        address shipOwner,
        string calldata _name,
        string calldata _symbol,
        ...
        // 0. Collect the launch fee from the CALLER (relayer/payer) into THIS
        //    contract. Exact amount, reverts fast on failure. fee=0 = free launch.
        //    The fee is split later: buy-in (min(buyInAmount,fee)) → prizeWallet
        //    bag; remainder → treasury (ops cash).
        ...
        //    usdc + treasury fund the paid cosmetic mutiny microtransaction.
        ...
        //     Distributor mints all 100 NFTs to shipOwner in its constructor.
        FeeShareDistributor dist = new FeeShareDistributor(
            ...
            "" // V1 Shipyard — empty baseURI; ShipyardV2 supplies the crew-meta URI
        );
        ...
        //     Never reverts the launch (failure routes the buy-in USDC to treasury).
    ...
    /// @dev Spends `bi = min(buyInAmount, fee)` USDC (already held by this
    ///      contract) buying TOKEN at the floor wall → prizeWallet, and sends the
    ///      `fee - bi` remainder to treasury. On ANY buy-in failure the `bi` USDC
    ///      is routed to treasury instead (no funds lost) and the launch never
    ///      reverts. No empty catch — every failure emits BuyInFailed.
    function _buyInAndSettleFee(address tokenAddr, uint256 fee) internal {
        ...
        if (bi > 0) {
            try this.executeBuyIn(tokenAddr, bi) returns (uint256 tokenOut) {
                ...
                // Buy-in failed: route the buy-in USDC to treasury (never lost).
    ...
    ///      (gives an atomic, revert-isolated buy-in). Only callable by self.
    ///      USDC → Money (deposit, 1:1) → TOKEN (slippage-capped swap) → prizeWallet.
    function executeBuyIn(address tokenAddr, uint256 usdcIn) external returns (uint256 tokenOut) {
        require(msg.sender == address(this), "internal only");

        // 1. USDC → Money (1:1). Approve EXACT, never MaxUint256.
        ...
        // 2. Money → TOKEN through the floor wall, slippage-capped.
        ...
        tokenOut = ISwapRouter02(swapRouter).exactInputSingle(
            ISwapRouter02.ExactInputSingleParams({
                ...
                tokenOut:          tokenAddr,
                fee:               FEE_TIER,
                recipient:         prizeWallet,   // moonshot bag goes straight out
                amountIn:          moneyBal,
                amountOutMinimum:  0,             // protected by sqrtPriceLimitX96
    ...
    /// @notice Owner sets the moonshot buy-in (USDC). Effective spend per launch
    ///         is min(buyInAmount, launchFee), so this can't exceed the fee.
    ...
    ///         Separate from treasury (ops cash).
