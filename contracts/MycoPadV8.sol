...
/// ⚠️ SUPERSEDED by MycoPadV9 / SporeReactorV6 — kept for reference, DO NOT DEPLOY.
///    This factory wires the "pay launcher 50% in the launched token" V5 reactor.
///    Replaced by V9 (USDC-paid fee-share via Money redemption). Diff history only.
///
/// @title Unrugable V8 — Free Launch, 2 Pools, 1 Reactor, Fee-Share NFTs
///
/// Clone of V7 with one addition: at launch it deploys a FeeShareDistributor
/// (100 NFTs, all minted to the launcher), and wires the V5 reactor to send
/// its launcher-50% of token fees into that distributor.
///
...
///
/// IMPORTANT: reactorImpl passed to this factory MUST be a SporeReactorV5 impl
/// (it exposes setDistributor). NEW LAUNCHES ONLY — does not touch V7/V4.
...
contract UnrugableV8 {
    ...
    address public immutable reactorImpl;      // MUST be a SporeReactorV5 impl
    ...
    function addPoolToReactor(address reactor, uint256 tokenId) external {
        ...
        ISporeReactorV5(reactor).addPool(tokenId);
    }
    ...
    function transferReactorAdmin(address reactor, address newAdmin) external {
        ...
        ISporeReactorV5(reactor).transferAdmin(newAdmin);
    }
