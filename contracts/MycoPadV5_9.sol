...
/// @title Unruggable2 V5.9 — Simplified: USDC → mftUSD → MfT Floor + mftUSD Sell Walls
///
/// Two launch modes:
...
///   4. Creates 1 floor pool: TOKEN/MfT (full range)
...
contract Unruggable2 {
    ...
    // Token pricing: how many tokens per 1 MfT of floor value
    // mftUSD has 6 decimals, MfT has 18 decimals
    // We derive token count from the MfT amount we receive from the swap
    uint256 constant FLOOR_TOKEN_RATIO = 10;   // 10 tokens per 1 MfT in floor
    ...
    function _launchStep1(
        ...
        // 4. Split MfT: 94% floor, 6% CHAR reactor
        uint256 floorMft = mftReceived * SEED_FLOOR / 10000;
        uint256 charMft  = mftReceived - floorMft;

        // 5. Create TOKEN/MfT floor pool (full range)
        uint256 floorTokens = poolSupply / 2;       // 50% of pool supply to floor
        uint256 floorMftId = _createFloorPool(tokenAddr, mft, floorTokens, floorMft);

        // 6. Create mftUSD sell walls with remaining supply
        //    Walls are one-sided (token only), priced relative to the floor
    ...
    function _createMftUsdWalls(
        ...
        // floor has floorTokens TOKEN per floorMft MfT
        // MfT price in mftUSD comes from the mftUSD/MfT pool
