...
///   Step 2 — launchStep2(): Create CHAR reactor + primary reactor,
///            deploy VolumeVesting clone, lock positions.
///
/// Reactor chain: token reactor -> CHAR reactor -> prime / invitation upstream
///
/// V5.6 changes from V5.5:
///   - 20% of token supply goes into a VolumeVesting clone for the launcher
///   - Vesting unlocks proportional to reactor burns (5000 BPS = 0.5 tokens per 1 burned)
///   - Launcher fully vests when cumulative burns reach 40% of total supply
///   - vestingOf mapping tracks vesting contract per token
...
contract Unruggable2 {
    ...
    function launchStep1(
        ...
        // V5.5: accept any contract address as upstream (cross-factory referrals)
        ...
        // V5.6: reserve 20% for launcher vesting, 80% for pools
        uint256 vestingAmount = _totalSupply * VEST_BPS / 10000;
        uint256 poolSupply = _totalSupply - vestingAmount;
        ...
        require(totalFloorTokens < poolSupply);
    ...
    function launchStep2(
        ...
        // V5.6: deploy vesting clone for launcher
        uint256 vestingAmount = p.totalSupply * VEST_BPS / 10000;
        address vestingAddr = _setupVesting(p.token, msg.sender, vestingAmount);
        ...
        launches.push(Launch({
            ...
            seed: p.usdcAmount, timestamp: block.timestamp
        }));

        emit TokenLaunched(p.token, reactorAddr, charReactorAddr, vestingAddr, msg.sender, _name, _symbol, p.totalSupply, p.usdcAmount);
    }

    // ═══════════════════════════════════════════════════════════════════════
    //  Internal — deploy VolumeVesting clone for launcher
    // ═══════════════════════════════════════════════════════════════════════
    ...
    function _setupVesting(
        ...
        IVolumeVesting(vestingAddr).initialize(
            tokenAddr,
            launcher,       // beneficiary = the launcher
            BURN,           // same burn address as reactors
            vestingAmount,
            VEST_RATE
        );
    }
    ...
    function addPoolToReactor(address reactor, uint256 tokenId) external {
        require(msg.sender == owner || msg.sender == launcherOf[reactor]);
