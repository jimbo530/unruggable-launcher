...
/// @title Unruggable2 — Unruggable Token Launcher (BTC/ETH/AZUSD floors)
///
/// Two-step launch:
...
///   - CHAR bought directly via USDC/CHAR pool, not routed through WETH
///   - cbBTC bought directly via USDC/cbBTC pool, not via WETH
///   - CHAR reactor reduced to 2 pools (CHAR/cbBTC + CHAR/WETH), no CHAR/MfT
///   - Removed _buyMft — no longer needed
///   - Step 2 now uses 3 swaps (was 8 in V5.1)
///   - Removed dead immutables (charUpstream, wethBtcFee)
///   - CHAR/USDC and USDC/cbBTC fee tiers are configurable immutables
///   - reactorOf / charReactorOf keyed by token address (not launcher)
///   - Pending state deleted before external calls in Step 2
...
contract Unruggable2 {
    ...
    mapping(address => address) public reactorOf;       // token -> primary reactor
    ...
    function launchStep2(
        ...
        // Delete pending before external calls (reentrancy safety)
    ...
    function _prepareFloorSeed(uint256 usdcAmount) internal returns (
        ...
        uint256 usdcForAzusd = usdcAmount * SEED_AZUSD / 10000;  // 40%
        uint256 usdcForBtc   = usdcAmount * SEED_BTC / 10000;     // 30%
        uint256 usdcForWeth  = usdcAmount - usdcForAzusd - usdcForBtc; // 30%

        // 1. USDC -> AZUSD via Aerodrome
    ...
    function _setupCharReactor(uint256 charUsdc, address upstreamAddr) internal returns (address charReactorAddr) {
        ...
        // Split CHAR and remaining USDC for 2 pools
        ...
        // Pool 1: CHAR/cbBTC — buy cbBTC directly from USDC
        IERC20(usdc).approve(swapRouter, usdcForBtc);
        ...
        // Pool 2: CHAR/WETH — buy WETH directly from USDC
