...
const AMT = ethers.parseUnits('50000', 18); // 5% each
...
async function main() {
  ...
  // BB(0xc70D) < EB(0xF3fC) → BB=token0, EB=token1
  // Both 18 dec, both $1. Full range 50K + 50K at 1:1
  console.log('\n=== BB / EB (BB=token0) ===');
  console.log('Creating pool...');
  await (await factory.createPool(BB, EB, FEE, {gasLimit: 5000000})).wait();
  await sleep(3000);
  const poolAddr = await factory.getPool(BB, EB, FEE);
  ...
  console.log('Minting 50K BB + 50K EB full range...');
  const tx = await npm.mint({
    token0: BB, token1: EB, fee: FEE,
  ...
  console.log('BB/EB cross NFT #' + nftId(r));
