...
async function main() {
  ...
  // BB(0xc70D) vs AZUSD(0x3595): 0x3595 < 0xc70D → AZUSD=token0, BB=token1
  // Both 18 dec. Init at $1. Sell BB (token1) below: [-887220, -120]
  console.log('\n=== AZUSD / BB (AZUSD=token0) ===');
  console.log('Creating pool...');
  await (await factory.createPool(AZUSD, BB, FEE, {gasLimit: 5000000})).wait();
  await sleep(3000);
  let poolAddr = await factory.getPool(AZUSD, BB, FEE);
  ...
  console.log('Minting 50K BB wall [-887220, -120]...');
  ...
  let r = await tx.wait();
  console.log('AZUSD/BB wall NFT #' + nftId(r));
  await sleep(3000);

  // EB(0xF3fC) vs AZUSD(0x3595): 0x3595 < 0xF3fC → AZUSD=token0, EB=token1
  // Both 18 dec. Init at $1. Sell EB (token1) below: [-887220, -120]
  console.log('\n=== AZUSD / EB (AZUSD=token0) ===');
  console.log('Creating pool...');
  ...
  console.log('Minting 50K EB wall [-887220, -120]...');
  tx = await npm.mint({
  ...
  r = await tx.wait();
  console.log('AZUSD/EB wall NFT #' + nftId(r));
