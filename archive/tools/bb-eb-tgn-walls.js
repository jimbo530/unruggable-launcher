...
const AMT = ethers.parseUnits('10000', 18); // 1% of 1M
...
async function main() {
  ...
  // === BB/TGN: BB(0x0a8D) < TGN(0xD75d) → BB=token0, TGN=token1 ===
  // Both 18 dec, both $1. Init at price=1.0. Sell BB above: [120, MAX_TICK]
  console.log('\n=== BB / TGN (BB=token0) ===');
  ...
  if (poolAddr === ethers.ZeroAddress) {
    ...
    await sleep(3000);
    poolAddr = await factory.getPool(BB, TGN, FEE);
  ...
  // === TGN/EB: TGN(0xD75d) < EB(0xeF1C) → TGN=token0, EB=token1 ===
  // Both 18 dec, both $1. Init at price=1.0. Sell EB (token1) below: [-MAX_TICK, -120]
  console.log('\n=== TGN / EB (TGN=token0) ===');
