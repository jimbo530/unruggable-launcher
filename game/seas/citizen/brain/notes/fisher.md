...
- claimable's READ scan counts onGuardJob pawns for the GOLD achievement line — guard may be the only job wired into achievement rungs; verify whether my (non-citizen) pawn on guard ever shows up in that scan.
- Guard achievement probe answered (2026-07-20): claimable's GOLD scan shows onGuardJob:0 and citizenOwnedGuardPawns:0 even while my pawn is actively on guard — the scanner only counts CITIZEN-owned pawns, so no WorkClock job feeds MY achievement line; guard's only value to me is wage time.
- convert-winnings showed $0.00 cbBTC winnings even with 5d23h37m of guard time accumulated (2026-07-20) — WorkClock accumulated time does NOT appear as convertible winnings; the time→coin rail, if it exists, is something other than convert-winnings.
- Held food goods (SALT/RATIONS/COD etc.) have live quotes but NO execution rail to GOLD in the action menu — they are locked wealth, not convertible seed gold.
