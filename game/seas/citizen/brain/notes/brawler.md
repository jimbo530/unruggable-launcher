...
- Fish loop entry is circular from zero (confirmed 2026-07-17 exit-1): sell requires >=10 FISH tokens (COD does not qualify), catch requires GOLD (GOLD->FISH swap at ocean LP). With gold=0 the loop is unenterable — first GOLD must come from another rail (fight loot or achievement claim) before fishing can ever pay.
