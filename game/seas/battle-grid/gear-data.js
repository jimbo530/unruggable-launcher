...
// ── ENCUMBRANCE: authentic D&D 3.5 item weights (lb), keyed to the tables above. Weight is
// per TYPE — material (wood/iron/bronze/steel) does NOT change it (a steel longsword weighs the
// same as an iron one), so it applies to every material tier of an item. Used by weight.js
// (pawn carry = 50 + STR; coins weigh 100/lb) to compute Light→Laden→Overloaded load.
  ...
  for (const [key, w] of Object.entries(WEAPONS)) {
    if (TIERED.has(key)) {
      ...
      add(key, w, w.name, weaponMods(w, 1, false, 0), priceCp(w.gp, 1),
        { material: 'iron', locked: false, masterwork: false, enchantable: false, enchant: 0, weight: WEIGHTS[key] });
    }
  }
  // armors
  for (const [key, a] of Object.entries(ARMORS)) {
    if (a.tiered) {
      ...
      add(key, a, a.name, armorMods(a, 0, false, 0), priceCp(a.gp, 1),
        { material: 'cloth', locked: false, masterwork: false, enchantable: false, enchant: 0, weight: WEIGHTS[key] });
    }
  }
  // trinkets
