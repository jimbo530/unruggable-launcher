...
//
//  SPECIES (ported in 2026-06-26): a SPECIES swaps the BASE BODY sprite for a whole
//  crew (one ship = one species). It is resolved per-crew from the ship (crewKey ->
//  ship-species.js) or an explicit look.species override, and falls back to the
//  acorn body when the chosen species' art is missing (asset-manifest.speciesBodyRel).
//  Everything else (aura, worn items, stickers, flags) is species-independent and
//  unchanged. acorn remains the ultimate, always-present fallback.
// ============================================================
const fs = require('fs');
...
// SPECIES resolution (per-ship crew body). speciesBodyRel does the on-disk
// existence check + acorn fallback; speciesForCrewKey maps a crewKey -> species id.
...
// Stat-driven AURA palette + colour (silver when balanced). Hue = dominant stat;
// brightness/size = imbalance score. See aura.js for the on-chain stat source.
const { AURA_COLORS } = require('./aura');
...
// cached per (resolved species + gender). Falls back to the placeholder if even the
// resolved asset is missing. Cached by "<species>:<gender>" (post-fallback species,
// so the acorn body is shared across any species that fell back to it).
...
// ── AURA halo (layer 0, BOTTOM): a soft feathered radial glow behind the doll.
//    hue        = dominant stat colour (silver when balanced)
//    brightness = imbalance score (0=generalist→faint silver wash, 1=specialist→vivid)
//    Built as an SVG radial gradient, then gaussian-blurred for a glow (not a disc).
//    Sized to read at 44px battle tiles AND 1000px crew tiles (geometry is fractional).
//
// IMBALANCE → GLOW curve (clearly-labeled; easy to retune):
//   We ease the raw 0..1 imbalance so mid-range pawns still show meaningful colour,
//   then map it onto opacity, radius, and a silver→hue colour blend. A small floor
//   keeps even-spread/silver visible at 44px (never fully invisible).
function auraTuning(imbalance) {
  const t = Math.min(1, Math.max(0, imbalance == null ? 0 : imbalance));
  const eased = Math.pow(t, 0.7);            // gentle ease-in so mid pawns read
  return {
    // Core opacity of the glow centre. Floor 0.34 (silver still visible) → 0.92 (vivid).
    opacity: 0.34 + eased * 0.58,
    // Glow radius as a fraction of canvas. Specialists glow slightly LARGER.
    radiusFrac: 0.46 + eased * 0.10,         // 0.46 (tight silver) → 0.56 (big vivid)
    // Blend toward the pure stat hue as imbalance climbs; balanced stays near silver.
    hueMix: eased,                           // 0 = silver, 1 = full stat colour
    // Inner-core saturation boost: brighter, more saturated centre when specialized.
    coreBoost: 0.30 + eased * 0.55,          // alpha of the bright inner core stop
  };
}

async function buildAura(dominant, imbalance) {
  const hue = AURA_COLORS[dominant] || AURA_COLORS.BALANCE;
  const silver = AURA_COLORS.BALANCE;
  const tune = auraTuning(imbalance);

  // Blend silver→hue by hueMix so balanced pawns read silver and specialists read vivid.
  const mix = (aHex, bHex, k) => {
  ...
  const glowColor = mix(silver, hue, tune.hueMix);

  const R = Math.round(tune.radiusFrac * CANVAS);   // glow radius in px
  const cx = Math.round(CANVAS / 2);
  // Centre the glow on the doll's body mass (sprite box sits low on the canvas).
  const cy = Math.round(SPRITE_TOP + SPRITE_BOX_H * 0.46);
  const D = R * 2;

  // Radial gradient: bright core → mid glow → transparent edge. Feathered + blurred.
  const svg = Buffer.from(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${CANVAS}" height="${CANVAS}">
       <defs>
         <radialGradient id="g" cx="50%" cy="50%" r="50%">
           <stop offset="0%"  stop-color="${glowColor}" stop-opacity="${tune.coreBoost.toFixed(3)}"/>
           <stop offset="38%" stop-color="${glowColor}" stop-opacity="${(tune.opacity * 0.85).toFixed(3)}"/>
           <stop offset="70%" stop-color="${glowColor}" stop-opacity="${(tune.opacity * 0.40).toFixed(3)}"/>
  ...
  // Blur softens the gradient banding into a true glow. Blur scales with radius so it
  // stays proportional at any output size (and survives downscale to 44px).
  const blur = Math.max(8, Math.round(R * 0.10));
  const buf = await sharp(svg).blur(blur).png().toBuffer();
 ...
 *          aura:{ dominant:'STR'|...|'BALANCE', imbalance:0..1 },
 *          crewKey?, species? }
 *
 * SPECIES (per-ship crew body) selection, in priority order:
 *   1. look.species  — explicit override (e.g. preview tooling / species cards)
 ...
 * present yet (asset-manifest.speciesBodyRel); the selection above is unaffected.
 */
async function renderCrew(look = {}) {
  ...
  // ---- layer 0 (BOTTOM): stat-driven AURA halo, UNSHIFTED behind the doll ----
  // look.aura is injected by the server from the on-chain dominant-stat lookup. When
  // absent (e.g. metadata-only callers), default to a faint balanced silver wash so
  // the layer is harmless rather than throwing.
  if (look.aura !== null) {
    const dominant = (look.aura && look.aura.dominant) || 'BALANCE';
    const imbalance = look.aura && typeof look.aura.imbalance === 'number' ? look.aura.imbalance : 0;
    const aura = await buildAura(dominant, imbalance);
    composites.unshift(aura); // bottom-most: glow renders BEHIND base + worn items
    traits.push({ trait_type: 'Aura', value: dominant === 'BALANCE' ? 'Balanced' : dominant });
  }
